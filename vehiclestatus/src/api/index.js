export async function getVechicleStatus(pageSize,filter) {
  ...
  export async function getVechicleStatusDetail(statusID) {
    const res = await fetch(`${process.env.DIAG_SERVICE_URL}/vehicleStatus/${statusID}`,{cache:'no-store'})
    // The return value is *not* serialized
    // You can return Date, Map, Set, etc.
